...
class RecipeListVC: UIViewController {
    ...
    private lazy var dataSource = DataSource(tableView: self.tableView) {
        (tableView, indexPath, item) -> UITableViewCell? in
        let cell = tableView.dequeueReusableCell(
            withIdentifier: RecipeListVC.reuseID,
            for: indexPath
        )
    ...
    init(folderId: ObjectID) {
        ...
        self.isRoot = folderId == StateManager.shared.root
    ...
    func updateItemsForSearchText() {
        guard let text = self.searchController.searchBar.text?.trim() else {
            return
        }
        if text.isEmpty {
            self.searchResults = self.items
        } else {
            self.searchResults = StateManager.shared.itemsMatchingText(text)
                .sorted(by: RecipeItem.sort)
        }
    }

    func applySnapshot(
        reload identifiers: [RecipeItem] = [],
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if StateManager.shared.userChanged {
            StateManager.shared.userChanged = false
            if self.isRoot {
                self.folderId = StateManager.shared.root!
            } else {
    ...
    private func configureViewController() {
        ...
        self.addRecipeButton = self.createBarButton(
            image: SFSymbols.add,
            action: #selector(self.addNewRecipe)
        )
        self.addFolderButton = self.createBarButton(
            image: SFSymbols.folder,
            action: #selector(self.addNewFolder)
        )
        self.menuButton = self.createBarButton(
            image: SFSymbols.contextMenu,
            menu: self.createMenuButtonContextMenu()
        )
        self.doneButton = self.createBarButton(
            image: SFSymbols.checkmarkCircle,
            action: #selector(self.disableEditMode)
        )
        self.moveButton = self.createBarButton(
            image: SFSymbols.folder,
            action: #selector(self.moveSelectedItems)
        )
        self.moveButton.isEnabled = false
        self.deleteButton = self.createBarButton(
        ...
        self.navigationItem.rightBarButtonItems = [
            self.menuButton, self.addFolderButton, self.addRecipeButton,
        ]
    }
    ...
    private func configureTableView() {
        ...
        self.tableView.register(
            ...
            forCellReuseIdentifier: RecipeListVC.reuseID
        )
    }
    ...
    private func createMenuButtonContextMenu() -> UIMenu {
        let recipeMenuItems = [
            UIAction(
                ...
                image: SFSymbols.addRecipe,
                handler: self.addNewRecipe
            ),
            UIAction(
                title: "Import recipe",
                image: SFSymbols.link,
                handler: self.importRecipe
            ),
        ]
        let folderMenuItems = [
            UIAction(
        ...
        let editMenuItems = [
            UIAction(
                ...
                image: SFSymbols.checkmarkCircle,
                handler: self.enableEditMode
            )
        ]

        let recipeMenu = UIMenu(
            title: "",
            image: nil,
            identifier: nil,
            options: .displayInline,
            children: recipeMenuItems
        )
        ...
        return UIMenu(
            ...
            identifier: nil,
            options: [],
            children: [recipeMenu, folderMenu, editMenu]
        )
    }

    private func createRecipeLongPressContextMenu(recipe: Recipe)
        -> UIContextMenuConfiguration
    {
        ...
        { [weak self] (_) in
            ...
            return UIMenu(
                title: "",
                children: [editAction, moveAction, cookAction, deleteAction]
            )
        }
    }

    private func createFolderLongPressContextMenu(folder: Folder)
        -> UIContextMenuConfiguration
    {
        return UIContextMenuConfiguration(identifier: nil, previewProvider: nil)
        { [weak self] (_) in
            ...
            return UIMenu(
                title: "",
                children: [editAction, moveAction, deleteAction]
            )
        }
    }
    ...
    func insertItem(item: RecipeItem) {
        ...
        let pos =
            self.items.firstIndex { RecipeItem.sort(item, $0) }
            ?? self.items.endIndex
    ...
    func removeItems(uuids: [ObjectID]) {
    ...
    @objc func addNewFolder(_ action: UIAction) {
        let alert = TextFieldAlert(
            ...
            Task {
                do {
                    let folder = try await StateManager.shared.addFolder(
                        name: text,
                        parent: folderId
                    )
    ...
    func editFolder(_ action: UIAction, folder: Folder) {
        let alert = TextFieldAlert(
            title: "Edit Folder",
            message: "Enter a name for this folder",
            placeholder: "Name",
    ...
    @objc func enableEditMode(_ action: UIAction? = nil) {
        ...
        self.navigationItem.rightBarButtonItems = [
            self.doneButton, self.moveButton, self.deleteButton,
        ]
    }
...
extension [RecipeItem] {
    ...
    func findItem(uuid: ObjectID) -> IndexPath? {
