...
struct NYTRecipeParser {
    ...
    static private func parseIngredients(_ document: Document) throws
        -> [Ingredient]
    {
        let listItems = try document.select("li.pantry--ui")
        // down-select to list items with class ingredient_ingredient__*
        let ingredientItems = try listItems.filter {
            try self.isIngredientListItem($0)
        }

        var ingredients: [Ingredient] = []
        for item in ingredientItems {
            let text = try item.children().map { try $0.text() }.joined(
                separator: " "
            )
    ...
    static private func parseInstructions(_ document: Document) throws
        -> [String]
    {
        let listItems = try document.getElementsByTag("li")
        // down-select to list items with class preparation_step__*
        let instructionItems = try listItems.filter {
            try self.isInstructionListItem($0)
        }
        ...
        for item in instructionItems {
            ...
            if paragraphs.isEmpty() {
                throw BasilError.recipeParseError(
                    "Failed to find instruction text"
                )
            }
            let text = try paragraphs.map { try $0.text() }.joined(
                separator: " "
            )
